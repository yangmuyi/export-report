...
public class MonthReportTask implements Runnable {
	...
	private static Logger log = LoggerFactory.getLogger(MonthReportTask.class);
	private String exportDate;
	private OrganVo organVo;
	private List<FormVo> formVoList;

	public MonthReportTask(String exportDate, OrganVo organVo,
		...
		try {
			for (int reportCount = 0; reportCount < formVoList.size(); reportCount++) {
				...
				try {
					...
					String filepath = "/conf/monthReportExportFailedReport.txt";
			...
			// 记录这家机构导出成功
			String filepath = "/conf/monthReportExportSuccessOrgan.txt";
			StringBuffer content = new StringBuffer();
