 ...
 * @date 2018-07-11
 */
public class SpanReportTask implements Runnable {

	private static Logger log = LoggerFactory.getLogger(SpanReportTask.class);
	private String ticket = "";
	private String exportDate;
	private OrganVo organVo;
	private List<FormVo> formVoList;

	public SpanReportTask(String exportDate, OrganVo organVo,
	...
	public void run() {
		try {
			for (int reportCount = 0; reportCount < formVoList.size(); reportCount++) {
				...
				if (additional[0] != null) {
					String conditionValue = additional[0].replace(
							...
							"${export_date}", exportDate.replace("|", " - "));
				...
				try {
					...
					String filepath = "/conf/spanReportExportFailedReport.txt";
			...
			// 记录这家机构导出成功
			String filepath = "/conf/spanReportExportSuccessOrgan.txt";
			StringBuffer content = new StringBuffer();
