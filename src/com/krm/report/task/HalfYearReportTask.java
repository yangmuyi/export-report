 ...
 * @date 2018-07-11
 */
public class HalfYearReportTask implements Runnable {
	...
	public HalfYearReportTask(String exportDate, OrganVo organVo,
	...
	public void run() {
		try {
			for (int reportCount = 0; reportCount < formVoList.size(); reportCount++) {
				...
				if (additional[0] != null) {
					String conditionValue = additional[0].replace(
							...
							"${export_date}",
							HandelReportDateUtil.getHalfYear(exportDate));
				...
				try {
					...
					String filepath = "/conf/halfyearReportExportFailedReport.txt";
			...
			// 记录这家机构导出成功
			String filepath = "/conf/halfyearReportExportSuccessOrgan.txt";
			StringBuffer content = new StringBuffer();
